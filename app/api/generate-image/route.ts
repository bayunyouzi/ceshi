...
const buildGptTaskKey = (input: {
  ...
  modelName: typeof input.modelName === "string" ? input.modelName : "gpt-image-2"
});
...
const runGptImageTask = async (taskId: string, input: GptImageTaskInput, user: { id: string; email: string | null } | null) => {
  ...
  // 从数据库加载 GPT-Image-2 配置
  const [cfgGptKey, cfgGptEndpoint, cfgGptModel] = await Promise.all([
    getConfig('GPT_IMAGE2_API_KEY'),
    getConfig('GPT_IMAGE2_API_ENDPOINT'),
    getConfig('GPT_IMAGE2_MODEL'),
  ]);
  const validGptKey = INVALID_GPT_IMAGE2_API_KEYS.has(cfgGptKey) ? await getConfig('GROK_IMAGE_API_KEY') : cfgGptKey;
  ...
  try {
    ...
    const asyncEndpoint = hasReferenceImage
      ? cfgGptEndpoint.replace(/\/images\/generations\/?$/i, '/images/edits')
      : cfgGptEndpoint;
    ...
    const response = await fetch(asyncEndpoint, {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${validGptKey}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 从数据库加载配置（带缓存，性能影响极小）
    const [
      cfgGrokImgKey, cfgGrokImgEndpoint, cfgGrokImgModel, cfgGrokImg2ImgModel,
      cfgGptKey, cfgGptEndpoint, cfgGptModel,
      cfgVideoKey, cfgVideoEndpoint, cfgVideoModel,
      cfgDailyLimit, cfgCooldown
    ] = await Promise.all([
      getConfig('GROK_IMAGE_API_KEY'),
      getConfig('GROK_IMAGE_API_ENDPOINT'),
      getConfig('GROK_IMAGE_MODEL'),
      getConfig('GROK_IMG2IMG_MODEL'),
      getConfig('GPT_IMAGE2_API_KEY'),
      getConfig('GPT_IMAGE2_API_ENDPOINT'),
      getConfig('GPT_IMAGE2_MODEL'),
      getConfig('VIDEO_API_KEY'),
      getConfig('VIDEO_API_ENDPOINT'),
      getConfig('VIDEO_MODEL'),
      getConfigInt('FREE_IMG_DAILY_LIMIT', 100),
      getConfigInt('COOLDOWN_SECONDS', 10),
    ]);

    // 验证 GPT Key 有效性
    const validGptKey = INVALID_GPT_IMAGE2_API_KEYS.has(cfgGptKey) ? cfgGrokImgKey : cfgGptKey;
    ...
    const defaultApiKey = isVideo ? cfgVideoKey : (isGpt2Model ? validGptKey : (isImg2Img ? cfgGrokImgKey : cfgGrokImgKey));
    const defaultEndpoint = isVideo ? cfgVideoEndpoint : (isGpt2Model ? cfgGptEndpoint : (isImg2Img ? cfgGrokImgEndpoint : cfgGrokImgEndpoint));
    const defaultModel = isVideo ? cfgVideoModel : (isImg2Img ? cfgGrokImg2ImgModel : cfgGrokImgModel);

    let finalApiKey = isGpt2Model ? validGptKey : (apiKey || defaultApiKey);
    let finalEndpoint = isVideo
      ? (apiEndpoint || defaultEndpoint)
      : (isGpt2Model ? cfgGptEndpoint : normalizeEndpoint(apiEndpoint, defaultEndpoint, "image", isImg2Img));
    const finalModel = isVideo ? (modelName || cfgVideoModel) : (modelName || defaultModel);
    // 图生图：点击 GPT 模式走 gpt-image-2；未点击则使用 grok-imagine-image-edit
    const actualModel = isImg2Img && !isVideo
      ? (isGpt2Model ? finalModel : cfgGrokImg2ImgModel)
      : finalModel;
    ...
    if (canAutoSwitchImageKey) {
      ...
      finalApiKey = todayAutoImageCount < cfgDailyLimit ? cfgGrokImgKey : cfgGrokImgKey;
    }
        ...
        if (decoded && decoded.userId) {
          ...
          if (user) {
            ...
            if (user.lastImageGeneratedAt) {
              ...
              if (timeDiff < cfgCooldown) {
                const remaining = Math.ceil(cfgCooldown - timeDiff);
    ...
    try {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (canAutoSwitchImageKey && !didKeyRetry && isQuotaExhaustedError(response.status, lastErrorText)) {
          const fallbackKey = cfgGrokImgKey;
